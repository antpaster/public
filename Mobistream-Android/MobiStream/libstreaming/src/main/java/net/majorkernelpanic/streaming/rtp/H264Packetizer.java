...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
	...
	public void setStreamParameters(byte[] pps, byte[] sps) {
		...
		if (pps != null && sps != null) {
			...
			//Log.w(TAG, "STAPA:" + byteArrayToHex(stapa));
			//STAPA:18 00 08 67 42 80 1f e9 05 89 c8 00 00 68 ce 06 f2 
	...
	private void send() throws IOException, InterruptedException {
		...
		//	Log.i(TAG, "NAL units are preceeded by their length, we parse the length");
			fill(header,0,5);
			ts += delay;
		...
		//	Log.i(TAG, "NAL units are preceeded with 0x00000001");
			fill(header,0,5);
...
//				Log.e(TAG, "NAL units are not preceeded by 0x00000001");
		...
		//	Log.i(TAG, "Nothing preceededs the NAL units");
			fill(header,0,1);
			header[4] = header[0];
			ts = ((MediaCodecInputStream)is).getLastBufferInfo().presentationTimeUs*1000L;
			//ts += delay;
...
//			Log.d(TAG,"We send two packets containing NALU type 7 (SPS) and 8 (PPS) lengh:" + stapa.length);
		}
		...
		if (naluLength<=MAXPACKETSIZE-rtphl-2) {
			...
			//String head1 = byteArrayToHex(buffer);
			//Log.d(TAG,head1);
	...
	//		String head1 = byteArrayToHex(header);
	//		Log.d(TAG,head1);
			
			
//			String res = byteArrayToHex(buffer);
//			Log.d(TAG,res);
//			Log.d(TAG,"----- Single NAL unit - len:"+len+" delay: "+delay);
	...
	//		Log.d(TAG,"Large NAL unit => Split nal unit");
			// Set FU-A header
...
//		Log.w(TAG, "");
		while (sum<length) {
			...
			//len = is.read(temp, sum, length-sum);
