...
import net.majorkernelpanic.streaming.rtp.FakeInputStream;
...
public abstract class MediaStream implements Stream {
	...
	public static final byte MODE_FAKE_API = 0x07;
	...
	public static byte sSuggestedMode = MODE_MEDIARECORDER_API;
	public byte mMode, mRequestedMode;

	protected boolean mStreaming = false, mConfigured = false;
	protected int mRtpPort = 0, mRtcpPort = 0; 
	protected byte mChannelIdentifier = 1;
	...
	protected FakeInputStream fakeInputStream;
	...
	public void setOutputStream(OutputStream stream, byte channelIdentifier) {
		...
		mPacketizer.getRtpSocket().setOutputStream(stream, channelIdentifier);
	}
	...
	public synchronized void configure() throws IllegalStateException, IOException {
		...
		if (mPacketizer != null) {
			...
			Log.w(TAG, "ChannelIdentifier = " + mChannelIdentifier);
			mPacketizer.getRtpSocket().setOutputStream(mOutputStream, (byte)(mChannelIdentifier));
		}
		mMode = mRequestedMode;
		mConfigured = true;
	}
...
//			throw new IllegalStateException("No destination ip address set for the stream !");
	...
	public synchronized  void stop() {
		if (mStreaming) {
			try {
				if(mMode==MODE_FAKE_API) {
					if(null != fakeInputStream)
						fakeInputStream.close();
					fakeInputStream = null;
					mPacketizer.stop();
				}else if (mMode==MODE_MEDIARECORDER_API) {
	...
	protected abstract void encodeWithFaceCodec() throws IOException;
	/**
