...
import net.majorkernelpanic.streaming.video.FakeVideoStream;
...
public class SessionBuilder {
	...
	public final static int VIDEO_FAKE = 3;
	...
	private int mAudioEncoder = AUDIO_AAC;// AUDIO_AMRNB;
	...
	public Session build() {
		...
		case VIDEO_H264:
			...
			break;
		case VIDEO_FAKE:
			FakeVideoStream stream2 = new FakeVideoStream();
			session.addVideoTrack(stream2);
			break;
		}

		if (mVideoEncoder == VIDEO_H264 && session.getVideoTrack()!=null) {
			VideoStream video = session.getVideoTrack();
			video.setFlashState(mFlash);
			video.setVideoQuality(mVideoQuality);
			video.setSurfaceView(mSurfaceView);
			video.setPreviewOrientation(mOrientation);
			video.setOutputStream(mOutputStream, (byte)(0));
		}

		if (session.getAudioTrack()!=null) {
			AudioStream audio = session.getAudioTrack();
			audio.setAudioQuality(mAudioQuality);
//			audio.setOutputStream(mOutputStream, (byte) (1));
		}
		return session;
	}
	...
	public SessionBuilder setSocket(Socket socket) {
		this.socket = socket;
		return this;
	}
	
	public SessionBuilder setOutoutStream(OutputStream out) {
