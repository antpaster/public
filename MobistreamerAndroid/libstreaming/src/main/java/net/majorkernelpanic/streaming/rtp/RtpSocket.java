...
import net.majorkernelpanic.streaming.realm.RTPStackBuffer;
...
public class RtpSocket implements Runnable {
	...
	public static boolean isDoUpload = false;


	public String TAG2 = "XZ";
	...
	public RTPStackBuffer stackBuffer;
...
//		mBufferCount = 300; // TODO: readjust that when the FIFO is full
		...
		for (int i=0; i<mBufferCount; i++) {
			...
			mBuffers[i][0] = (byte) Integer.parseInt("10000000", 2);
				...
				mBuffers[i][0] = (byte) Integer.parseInt("10100000", 2);
			}

			/* Payload Type */
			mBuffers[i][1] = (byte) 96;   //второй байт RTP заголовка
...
//		storage.close();
	...
	public void setOutputStream(OutputStream outputStream, byte channelIdentifier) {
		if (outputStream != null) {
			...
			mReport.setOutputStream(outputStream, (byte) (channelIdentifier));
			//Log.w(TAG, "ChannelIdentifier = " + channelIdentifier);
			//mReport.setOutputStream(outputStream, (byte) (0));
		}
	}
...
//				mReport.update(mPackets[mBufferOut].getLength(), (mTimestamps[mBufferOut]/100L)*(mClock/1000L)/10000L);
				...
				if (mCount++>30) {
					if (mTransport == TRANSPORT_UDP) {
						...
						if(isAudio && TAG2.equalsIgnoreCase("AUDIO")) {
							sendTCP();
						}else if(!TAG2.equalsIgnoreCase("AUDIO")){
							sendTCP();
						}
					}
				}
	...
	private void sendTCP() {
		synchronized (MySemafor.semafor) {
			...
			if (isDoUpload) {
				len = mPackets[mBufferOut].getLength() + 9; // для дозагрузки
			}

			mTcpHeader[2] = (byte) (len>>8);
			mTcpHeader[3] = (byte) (len&0xFF);
			try {
				if (TAG2.equalsIgnoreCase("AUDIO")){
					mTcpHeader[1] = (byte) 2;
				}

				if(stackBuffer == null){
					Log.d(TAG,"new RTPStackBuffer(context);");
					stackBuffer = new RTPStackBuffer(context);

				}else{

				}

//				TCPClient.getInstance().writeToBuffer(mTcpHeader, mBuffers[mBufferOut], 0, len);

				byte [] arr = new byte[mTcpHeader.length + len];
				System.arraycopy(mTcpHeader, 0, arr, 0, mTcpHeader.length);
				System.arraycopy(mBuffers[mBufferOut], 0, arr, mTcpHeader.length,mPackets[mBufferOut].getLength());

				if (isDoUpload) {
					byte [] ts = new byte[8];
					ts[0] = (byte) ((mTimestamps[mBufferOut]>>56)&0xFF);
					ts[1] = (byte) ((mTimestamps[mBufferOut]>>48)&0xFF);
					ts[2] = (byte) ((mTimestamps[mBufferOut]>>40)&0xFF);
					ts[3] = (byte) ((mTimestamps[mBufferOut]>>32)&0xFF);
					ts[4] = (byte) ((mTimestamps[mBufferOut]>>24)&0xFF);
					ts[5] = (byte) ((mTimestamps[mBufferOut]>>16)&0xFF);
					ts[6] = (byte) ((mTimestamps[mBufferOut]>>8)&0xFF);
					ts[7] = (byte) (mTimestamps[mBufferOut]&0xFF);

					//byte[] bytes = ByteBuffer.allocate(Long.SIZE / Byte.SIZE).putLong(someLong).array();


					System.arraycopy(ts, 0, arr, mTcpHeader.length + mBuffers[mBufferOut].length, ts.length);

					arr[arr.length - 1] = (byte)9;

				}

//				byte [] t = new byte[4];
//				System.arraycopy(mBuffers[mBufferOut], 4, t, 0, 4);

	//			ByteBuffer wrapped = ByteBuffer.wrap(t); // big-endian by default
	//			Log.d(TAG,"TIMESTAMP: " + wrapped.getLong());

	//			Log.d(TAG,"TIMESTAMP: " + mTimestamps[mBufferOut]);


				stackBuffer.pushPacket(mTimestamps[mBufferOut], arr);

				//RTPStackStorage.getInstance().pushPacket(1, arr);
