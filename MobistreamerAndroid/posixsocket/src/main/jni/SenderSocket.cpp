...
bool SenderSocket::ConnectToAddress(int sd, const char* host, unsigned short p){
    ...
    if (0 == inet_aton(host, &(address.sin_addr))) {
        // Throw an exception with error number
  //      ThrowErrnoException(env, "java/io/IOException", errno);
        LogMessage("Not Connected to %s:%d Error:%d", host, p, errno);
    }else{
        address.sin_port = htons(p);
        // Connect to address
        if (-1 == connect(sd, (const struct sockaddr*) &address, sizeof(address))){
            LogMessage("Connect to address Error: %d", errno);
            // Throw an exception with error number
//            ThrowErrnoException(env, "java/io/IOException", errno);
        }else{
...
int SenderSocket::sendToHost(char *data, size_t len){

   // return errno;

    //   LogMessage("int SenderSocket::sendToHost(char *data, int size)");
    int sended = SendToSocket(connectedSocket, data,len);
...
int SenderSocket::readFromHost(char *data, size_t len){
     ...
     int readed = recv(connectedSocket, readData, 2048, 0);


     memcpy (data, readData, readed);
     ...
     while (bytesRead < len)
     {
         result = read(connectedSocket, readData + bytesRead, len - bytesRead);
